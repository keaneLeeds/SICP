;; Structure and Interpretation of Computer Programs
;; Chapter 1
;;
;; Exercise 1.37
;; a. An infinite continued fraction is an expression of the form
;;
;;             N1
;;      f = ----------
;;           D1 + N2 
;;              --------
;;              D2 + N3
;;                 ------
;;                  D3 + ...
;;
;; As an example, one can show that the infinite continued fraction expansion
;; with the Ni and the Di all equal to 1 produces 1/phi, where is the golden
;; ratio (described in section 1.2.2). One way to approximate an infinite
;; continued fraction is to truncate the expansion after a given number of
;; terms. Such a truncation -- a so-called k-term finite continued fraction --
;; has the form
;;
;;          N1
;;  f = -------------
;;      D1  +  N2
;;        .  --------
;;          .      Nk
;;            . + ---
;;                 Dk
;;              
;; Suppose that n and d are procedures of one argument (the term index i) that
;; return the Ni and Di of the terms of the continued fraction. Define a
;; procedure cont-frac such that evaluating (cont-frac n d k) computes the value
;; of the k-term finite continued fraction. Check your procedure by
;; approximating 1/phi using

(cont-frac (lambda (i) 1.0)
           (lambda (i) 1.0)
           k)

;; for successive values of k. How large must you make k in order to get an
;; approximation that is accurate to 4 decimal places?

;; b. If your cont-frac procedure generates a recursive process, write one that
;; generates an iterative process. If it generates an iterative process, write
;; one that generates a recursive process.

; recursive
; takes k = 11 to be accurate to 4 decimal places
(define (cont-frac n d k)
    (define (cont-frac-rec n d k i)
        (if (= k i)
            (/ (n k) (d k))
            (/ (n i) (+ (d i) (cont-frac-rec n d k (+ i 1))))))
    (cont-frac-rec n d k 1))

; iterative
(define (cont-frac n d k)
    (define (cont-frac-iter n d k sum)
        (if (= k 1)
            (/ (n 1) (+ (d 1) sum))
            (cont-frac-iter n d (- k 1) (/ (n k) (+ (d k) sum)))))
    (cont-frac-iter n d k 0))

