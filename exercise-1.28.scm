...
(define (expmod base exponent m)
  (cond ((= exponent 0) 1)
        ((even? exponent) (remainder (modulo1? (expmod base (/ exponent 2) m) m) m))
        (else (remainder (* base (expmod base (- exponent 1) m)) m))))

(define (modulo1? number n)
    (cond ((or (= number 1) (= number (- n 1))) (square number))
          ((= (modulo (square number) n) 1) 0)
          (else (square number))))
